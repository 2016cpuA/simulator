...
int convert_data(Instr_list *prepare,int *mem){
  ...
  while(!list_isempty(now)){
    ...
    if(dest.opcode==_WORD){
      ...
      if(data_hi&32768){ //下から16bit目が1
	if(data_hi&0xc0000000){
	  if((data_hi&0x80000000)&&(data_hi&0x40000000)){
	    //上位2bitがともに1 ->上位17bitを負の数として扱って16bitに圧縮(?)できる	    
	    data_lo=data_hi&0x7FFF;
	    data_hi=data_hi>>15;
	    list_pop(now);
	    ...
	    *mem+=align;
	    ...
	    //上位2bitが10か01になっている-> 超 面 倒 く せ ぇ
	    data_lo=data_hi&0x7FFF;
	    data_hi=data_hi>>15;
	    list_pop(now);
	    ...
	    ins.operands[0]=1;
	    ins.operands[1]=0;
	    ins.operands[2]=(data_hi&0x18000)>>15; //上位2bitを取り出す
	    ins.operands[3]=0;
	    list_push(now,ins);
	    now=now->next;
	    n_instr++;
